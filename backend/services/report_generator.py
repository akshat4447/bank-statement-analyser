...
def generate_pdf_report(analysis: AnalysisResponse, output_path: str) -> str:
    ...
    story.append(Paragraph("BSA Platform — Enterprise-Grade Financial Intelligence", small_style))
    story.append(HRFlowable(width="100%", thickness=2, color=NAVY, spaceAfter=12))

    # Account summary table
    # 🔴 BUG FIX #2 & #3: Use actual transaction dates (from analytics) and last transaction balance
    if ai:
        story.append(Paragraph("Account Information", h1_style))

        # Use transaction-derived dates instead of PDF header metadata
        actual_from = an.actual_period_from if an else (ai.statement_period_from or "—")
        actual_to = an.actual_period_to if an else (ai.statement_period_to or "—")
        ...
        if analysis.transactions:
            ...
            if sorted_txns and sorted_txns[-1].balance is not None:
                ...
                last_txn_balance = f"₹{ai.closing_balance:,.2f}" if ai.closing_balance else "—"
        elif ai.closing_balance:
            last_txn_balance = f"₹{ai.closing_balance:,.2f}"
        ...
        acct_data = [
            ...
            ["Statement From", actual_from, "Statement To", actual_to],
            ["Opening Balance", f"₹{ai.opening_balance:,.2f}" if ai.opening_balance else "—",
             "Closing Balance", last_txn_balance],
        ]
    ...
    story.append(Paragraph(
        f"Generated by BSA Platform · Developed by Akshat Kumar | {datetime.now().strftime('%d %B %Y %H:%M')}",
